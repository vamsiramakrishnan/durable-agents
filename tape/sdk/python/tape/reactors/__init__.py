...
def outbox_relay_tick(url: str, sink: Any, *, cursor_path: str = "",
    ...
    c = client or TapeClient(url)
    try:
        # Cursor is the global_seq (single int) — the new event-bus cursor
        # format. `_read_cursor` migrates legacy v1 cursors (which carried
        # `from_ts_ms` / `last_run_id` / `last_seq`) by resetting to 0; the
        # downstream sink dedupes on (run_id, seq), so a one-time re-read is
        # harmless. The subject pattern + predicate are forwarded to the
        # server-side SubscribeBySubject so filtering happens at the source.
