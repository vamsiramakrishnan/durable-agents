...
def run(
    session_id: str = typer.Argument(..., help="ADK session id to inspect."),
    app_name: str = typer.Option(
        ..., "--app", help="ADK app name (the `name=` on App)."),
    user_id: str = typer.Option(..., "--user", help="ADK user id."),
    ...
    """Snapshot a single ADK session's journal — events, effects, obligations,
    timers — from the tape-adk SQLAlchemy store.

    For multi-run-fleet triage use `tape doctor --live --db-url …`. For a
    live-tailing TUI re-pointed at the embedded path, that's still on the
    roadmap; today this is the snapshot equivalent of `tape inspect <run>
    --print`.
    """
